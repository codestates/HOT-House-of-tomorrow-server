  ...
  if (!oAuthId || !email) {
    ...
    if (userInfo) {
      ...
      console.log("login.js token1:", token);
        res.status(200).cookie('x_auth', token, { httpOnly: true, sameSite: 'lax' }).json({ loginSuccess: true, token: token, userInfo: userInfo });
    } else {
      ...
      res.status(200).cookie('x_auth', token, { httpOnly: true, sameSite: 'lax' }).json({ loginSuccess: true, token: token, userInfo: newUser });
    }
  }
};
